...
func (g *GenVisitor) VisitCallExpression(n *ast.CallExpression) {
	...
	for i := range n.ArgumentList {
		...
		if i < len(n.ArgumentList)-1 {
			g.writeByte(',')
			g.space()
			g.printListGap(n.ArgumentList[i].Idx1(), n.ArgumentList[i+1].Idx0())
		}
	}
	g.writeByte(')')
...
func (g *GenVisitor) VisitClassLiteral(n *ast.ClassLiteral) {
	...
	if g.comments != nil {
		...
		if n.Name != nil {
			hi = n.Name.Idx
		} else if n.SuperClass != nil {
			hi = n.SuperClass.Idx0()
		}
		g.printGap(n.Class, hi)
	}
	if n.Name != nil {
		g.writeByte(' ')
		g.gen(n.Name)
...
func (g *GenVisitor) VisitArrayLiteral(n *ast.ArrayLiteral) {
	...
	for i, ex := range n.Value {
		...
		if i < len(n.Value)-1 {
			g.writeByte(',')
			g.space()
			if !ex.IsNone() && !n.Value[i+1].IsNone() {
				g.printListGap(n.Value[i].Idx1(), n.Value[i+1].Idx0())
			}
		}
	}
	g.writeByte(']')
...
func (g *GenVisitor) VisitObjectLiteral(n *ast.ObjectLiteral) {
	...
	for i := range n.Value {
		g.lineAndPad()
		if g.comments != nil {
			g.printGap(prevEnd, propertyStart(n.Value[i]))
		}
		g.gen(&n.Value[i])
		if i < len(n.Value)-1 {
			g.writeByte(',')
...
func (g *GenVisitor) VisitStatements(n *ast.Statements) {
	g.visitStmtList(*n, ast.Idx(len(g.src)))
}
